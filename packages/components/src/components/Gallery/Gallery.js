...
export default class Gallery extends Component {
  ...
  render() {
    ...
    const PlayPause = playing ? (
      <Icon icon="pause" className="pause" title="Pause" onClick={() => this.pause()} />
    ) : (
      <Icon icon="play" className="play" title="Play" onClick={() => this.play()} />
    );
    ...
    return (
      <ContainerOuter className={fullWidth ? 'fullwidth' : ''}>
        <ContainerInner itemCount={itemCount} currentItem={currentItem}>
          {items.map((item, idx) => (
            <GalleryImage key={idx} itemCount={itemCount} item={item} lightbox={lightbox} />
          ))}
        </ContainerInner>
